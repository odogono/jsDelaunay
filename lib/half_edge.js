...
HalfEdgePriorityQueue.prototype.insert = function(halfEdge) {
    var previous, next;
    debugger;
    var insertionBucket = Math.round(this.bucket(halfEdge));
...
HalfEdgePriorityQueue.prototype.remove = function(halfEdge){
    var previous;
    var removalBucket = Math.round(this.bucket(halfEdge));
...
HalfEdgePriorityQueue.prototype.bucket = function(halfEdge) {
    var theBucket = Math.round( (halfEdge.ystar - this.ymin)/this.deltay * this.hashSize );
...
HalfEdgePriorityQueue.prototype.isEmpty = function(bucket) {
    var key = Math.round(bucket);
    var entry = this.hash[key];
    return (entry.nextInPriorityQueue === null);
}
