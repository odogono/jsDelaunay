...
Site.prototype.isEqual = function(other){
    return this.siteIndex === other.siteIndex;
}
...
Site.prototype.nearestEdge = function() {
    this.edges.sort(Edge.compareSitesDistances);
    return this.edges[0];
}

Site.prototype.neighbourSites = function() {
    ...
    for( var i=0,len=this.edges.length;i<len;i++ ){
        list.push( this.neighbourSite(this.edges[i]) );
    }
    return list;
}
            
Site.prototype.neighbourSite = function(edge/*:Edge*/)/*:Site*/ {
    if (this.isEqual(edge.leftSite)) {
        return edge.rightSite;
    }
    if (this.isEqual(edge.rightSite)) {
